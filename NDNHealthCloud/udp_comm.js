...
*/
function handleInterestPacket(packetDataArray, replyIP, replyPort) {
	var nameComponent;
	var i;
	...
	if (allFIBData === null || allFIBData.length === 0) {
	    ...
	    for (i = 0; i < allFIBData.length; i++) {
	        ...
	        if (allFIBData[i].ipAddr !== packetIP) {
	            ...
	            sendMessage(dataPacket.toString(), nonPrivateIP, nonPrivatePort); // send interest packet
	        }
	    }
	}
}
                    ...
                    packetProcessID, packetIP, nonPrivateIP, nonPrivatePort)
{
	...
	if (csDATA !== null) {
	    ...
	    for (i = 0; i < csDATA.length; i++) {
	        ...
	        sendMessage(dataPacket.toString(), nonPrivateIP, nonPrivatePort); // reply to interest with DATA from cache
	    }
...
*/
function handleDataPacket (packetDataArray, ip)
{
